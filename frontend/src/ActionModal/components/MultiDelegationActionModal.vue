<template>
  <transition v-if="openModalId === transactionData.type" name="slide-fade">
    <div>
      <div class="multi-delegation-action-modal-overlay" @click="close"></div>
      <div v-focus-last class="multi-delegation-action-modal" tabindex="0" @keyup.esc="close">
        <div
          v-if="(step === feeStep || step === signStep) && !sending"
          id="prevBtn"
          class="multi-delegation-action-modal-icon multi-delegation-action-modal-prev"
          @click="previousStep"
        >
        ...
        <div
          id="closeBtn"
          class="multi-delegation-action-modal-icon multi-delegation-action-modal-close"
          @click="close"
        >
          <i class="material-icons">close</i>
        </div>
        <div class="multi-delegation-action-modal-header">
          <span class="multi-delegation-action-modal-title">{{
        ...
        <template v-else>
          <div v-if="requiresSignIn" class="multi-delegation-action-modal-form">
            <p class="form-message notice">
              You need to sign in to submit a transaction.
            </p>
          </div>
          <div v-else-if="step === defaultStep" class="multi-delegation-action-modal-form">
            <slot />
          </div>
          <div v-else-if="step === feeStep" class="multi-delegation-action-modal-form">
            <TmFormGroup
              v-if="session.experimentalMode"
              :error="$v.gasPrice.$error && $v.gasPrice.$invalid"
              class="multi-delegation-action-modal-group"
              field-id="gasPrice"
            ...
            <div style="margin: 50px 0;">
              <TmFormGroup
                ...
                class="multi-delegation-action-modal-form-group"
          ...
          <div v-else-if="step === inclusionStep" class="multi-delegation-action-modal-form">
            <TmDataMsg>
              <div slot="image">
                <img
          ...
          <div
            ...
            class="multi-delegation-action-modal-form success-step"
          >
            <TmDataMsg
          ...
          <div class="multi-delegation-action-modal-footer">
            <slot name="multi-delegation-action-modal-footer">
              <TmFormGroup
...
export default {
  name: `multi-delegation-action-modal`,
  components: {
    HardwareState,
    TmBtn,
    TmField,
...
<style lang="scss">
.multi-delegation-action-modal-overlay {
  position: fixed;
  top: 0;
  right: 0;
  width: 100vw;
...
.multi-delegation-action-modal {
  ...
  > .multi-delegation-action-modal-header {
    text-align: center;
    > .step--container {
      margin-top: var(--unit);
    }
  }
  > .multi-delegation-action-modal-form {
    .form-message {
      font-size: 12px;
    }
  }
  ...
  .multi-delegation-action-modal-group.tm-form-group,
  .multi-delegation-action-modal-form-group.tm-form-group {
    padding: 0;
    margin-bottom: var(--unit);
  }
  .multi-delegation-action-modal-group.tm-form-group:last-child {
    margin-bottom: 0;
  }

  .multi-delegation-action-modal-title {
    font-weight: bold;
  }

  .multi-delegation-action-modal-icon.multi-delegation-action-modal-close {
    cursor: pointer;
  }
}
/*
...
.multi-delegation-action-modal-icon {
  display: flex;
  align-items: center;
  justify-content: center;
}

.multi-delegation-action-modal-icon i {
  font-size: var(--lg);
}

.multi-delegation-action-modal-icon.multi-delegation-action-modal-prev {
  cursor: pointer;
  position: absolute;
  top: 1rem;
  left: 1rem;
...
.multi-delegation-action-modal-icon.multi-delegation-action-modal-close:hover i {
  color: var(--link);
}

.multi-delegation-action-modal-form .tm-form-group {
  display: block;
  padding: 1rem 0;
}

.multi-delegation-action-modal-footer {
  display: flex;
...
.multi-delegation-action-modal-footer .tm-form-group {
  padding: 0;
}
