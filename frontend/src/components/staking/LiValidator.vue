<template>
  ...
  >
    ...
    <td :class="{ 'hide-xs': showOnMobile !== 'expectedReturns' }">{{ 0.00005 | percent }}</td>
    <td :class="{ 'hide-xs': showOnMobile !== 'expectedReturns' }">{{ validator.rate | percent }}</td>
    <td>{{ validator.total_stake | ones | zeroDecimals }}</td>
    <td
      :class="{ 'hide-xs': showOnMobile !== 'expectedReturns' }"
    >{{ validator.voting_power | percent }}</td>
  </tr>
</template>
